...
void ps(Job* jobs[], int n){
    ...
    printf("Process \t Arrival Time \t Burst Time \t Response Time\t Start Time \t Completion Time \t Turnaround Time \t Waiting Time \n");
    for(int i =0;i<n;i++){
        printf("P%d \t\t %d \t\t %d \t\t %d \t\t %d \t\t %d \t\t\t %d \t\t\t %d\n",jobs[i]->pid+1, jobs[i]->arrival, jobs[i]->burst,
